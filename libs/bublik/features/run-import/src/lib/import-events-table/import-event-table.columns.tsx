...
export const columns = [
	columnHelper.accessor('status', {
		...
		cell: (cell) => {
			...
			return (
				<Tooltip content={status} side="right" align="start">
					<div
						className={cn(
							'h-[calc(100%+2px)] rounded-l w-[24px] flex flex-col items-center',
		...
		meta: { className: 'rounded-l p-0 overflow-hidden', width: '24px' }
	}),
		...
		cell: ({ row }) => {
			...
			return (
				<Tooltip content="Expand/Collapse">
					<button
						className="grid place-items-center p-1 rounded hover:bg-primary-wash text-primary"
						onClick={() => row.toggleExpanded()}
					>
						<Icon
							name="ArrowShortTop"
							className={cn('rotate-90', row.getIsExpanded() && 'rotate-180')}
						/>
					</button>
				</Tooltip>
			);
