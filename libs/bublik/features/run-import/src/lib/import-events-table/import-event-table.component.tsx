								...
								return (
									<div
										...
										className={cn(
											...
											'flex items-center',
											idx !== arr.length - 1 && !row.getIsExpanded() && 'mb-1',
				...
				{events.map((evt) => {
					...
					return (
						<div key={evt.event_id} className="relative">
							<div
								className={`absolute -left-[32px] top-[11px] flex size-9 items-center justify-center rounded-full ${bg} text-white`}
							>
							...
							<div className="space-y-3 rounded-lg ml-4 border border-border-primary bg-gray-50/40 p-4">
								...
								{evt.error_msg && (
									<p className="font-medium text-sm bg-primary-wash p-4 rounded-md">
										{evt.error_msg}
									</p>
								)}
							</div>
