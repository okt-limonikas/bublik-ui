...
interface GotoOptions {
	/** Raw block id; the page object encodes it the way the app does. */
	hash?: string;
	search?: string;
}

/** Report block ids carry `:` and `|`, which attribute selectors accept as-is. */
...
class RunReportPage {
	...
	// Scoped to the report itself: the admin sidebar carries its own "Configs" entry.
	...
	/**
	 * Content blocks are addressed by the DOM id the app derives from the block
	 * id — the same encoding the anchors use, so a change to it fails here first.
	 */
	...
	/**
	 * Mirrors scrollToReportItem: the gap between the block's top and the
	 * scroller's, after the sticky `[data-offset]` compensation. Zero means the
	 * page scrolled exactly where the app intended, so this fails the moment a
	 * sticky header offset or the id encoding regresses.
	 */
	...
	/** The scroller cannot bring the last blocks to its top; it clamps instead. */
	...
	/** Scrolling is smooth, so wait for consecutive identical samples. */
	...
	private async scrollAlignment(
		...
		// A block near the end of the report can only be scrolled as far as the
		// scroller allows, so being visible at the bottom is the correct outcome.
	...
	/**
	 * Alignment is only meaningful once the smooth scroll has stopped: a block
	 * that is still 10px short reads as "close enough" to the eye but is not yet
	 * the block j and k navigate from.
	 */
	...
	/* ------------------------------------------------------------------ *
	 * Keyboard and its on-screen equivalents
	 * ------------------------------------------------------------------ */

	/** The hotkeys listen on document and skip inputs, so nothing may be focused. */
	...
	/* ------------------------------------------------------------------ *
	 * Table cells and the log preview drawer
	 * ------------------------------------------------------------------ */
	...
	async openLogPreview(recordId: string, resultId: number): Promise<void> {
		// The table renders only once its record is observed, so reach the record
		// before looking for the cell.
	...
	/**
	 * The drawer's links open in a new tab, so the hrefs are what can be
	 * asserted — clicking them would leave the report behind.
	 */
	...
	/** Charts render only once scrolled near, so the button has to be reached. */
