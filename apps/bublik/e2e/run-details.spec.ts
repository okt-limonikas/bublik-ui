...
test.describe('Run Details Page (signed out)', () => {
	...
	test(
		...
		async ({ page }) => {
			...
			// Any row without a note offers the add trigger; rows other tests
			// have annotated show the edit trigger instead.
			const addNote = () =>
				page
					.getByTestId('run-note-cell')
					.getByRole('button', { name: 'Log in to add notes', exact: true })
					.first();
			...
			await then(
				...
				async () => {
					await expect(addNote()).toHaveText(/Add Note/, { timeout: 30_000 });
					await addNote().hover();
					await expect(
