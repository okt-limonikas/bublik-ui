...
/* Implements features/run-report.feature */
...
/**
 * Every scenario below drives its assertions from the report payload, so a
 * fixture change moves the tests with it instead of breaking them.
 */
...
test.describe('Run Report Page', () => {
	...
	test(
		...
		async ({ page }) => {
			...
			// Measurement entries start collapsed, so their records are mounted
			// only once the entry is opened.
	...
	test(
		...
		async ({ page }) => {
			...
			// The entry only carries `?config=`, so the other params are dropped —
			// the configuration is the invariant worth pinning here.
			await and(
	...
	test(
		...
		async ({ page }) => {
			...
			// Records render ten at a time until an anchor forces the whole list.
	...
	test(
		...
		async ({ page }) => {
			...
			// The key acts on the test block the scroller is currently inside, so
			// the report has to be scrolled into one first.
	...
	test(
		...
		async ({ page }) => {
			...
			// The preview is deliberately component state, not URL state.
